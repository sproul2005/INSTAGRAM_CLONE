          ...
          <span>{userProfile?.username}</span>
          {
            isLoggedInUserProfile ? (
              <>
                ...
                <Button variant='secondary' className='hover:bg-gray-200 h-8'>Ad tols</Button>
              </>  
            ) :(
              isFollowing? (
                <>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div className='flex flex-col gap-1'>
             <span className='font-semibold'>{userProfile?.bio || 'bio here...'}</span>
                <div className='flex items-center gap-1 bg-gray-200 px-4 py-2 rounded-lg w-fit'>
                   <Badge className='w-full bg-gray-200  ' variant='secondary'><AtSign/></Badge>
                   <span className='text-md font-semibold'>{userProfile?.username}</span>
                </div>
                ...
                <img src={post.image} alt='postimage' className='rounded-sm my-2 w-full aspect-square object-cover'/>
                  <div className='absolute inset-0 flex items-center justify-center  bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
                   <div className='flex items-center text-white space-x-1'>
                     <Button className='flex items-center gap-2  rounded hover:text-gray-300'>
                       <Heart/>
